...
int current_group = USER_PROC_GROUP_IO;
int counter_io = 1;
int counter_calc = 2;
int counter_inter = 4;
int a = 0;
 ...
 */
  ...
  register int previous_proc_group;
  register int new_proc_group;
  int i;
  ...
  if ( (rp = rdy_head[SERVER_Q]) != NIL_PROC) {
	...
	return;
	 ...
	 for (i = 0; i < USER_PROC_GROUP_COUNT; i++){
		new_proc_group = (current_group + i) % USER_PROC_GROUP_COUNT;
		if ( (rp = rdy_head[get_proc_group_queue(new_proc_group)]) != NIL_PROC) {
			proc_ptr = rp;
			bill_ptr = rp;
			current_group = new_proc_group;
			return;
	  }
	}
 ...
 */
  register int user_proc_group_queue;
   ...
   * bound processes.)
   */
  user_proc_group_queue = get_proc_group_queue(rp->p_usergroup);
  if (rdy_head[user_proc_group_queue] == NIL_PROC)
	rdy_tail[user_proc_group_queue] = rp;
  rp->p_nextready = rdy_head[user_proc_group_queue];
  rdy_head[user_proc_group_queue] = rp;
...
/* A process has blocked. */
  ...
  if (istaskp(rp)) {
	...
	user_proc_group_queue = get_proc_group_queue(rp->p_usergroup);
		if ( (xp = rdy_head[user_proc_group_queue]) == NIL_PROC) return;
		if (xp == rp) {
				rdy_head[user_proc_group_queue] = xp->p_nextready;
	...
	qtail = &rdy_tail[user_proc_group_queue];
  }
 ...
 */
  register int current_proc_group;
	register int user_proc_group_queue;
	register int new_proc_group;
	int i;

  /* if (rdy_head[USER_Q] == NIL_PROC) return; */
	if (rdy_head[USER_Q_IO] == NIL_PROC
		&& rdy_head[USER_Q_CALC] == NIL_PROC
		&& rdy_head[USER_Q_INTER] == NIL_PROC ) {
		return;
	}

	user_proc_group_queue = get_proc_group_queue(current_group);
	if (rdy_head[user_proc_group_queue] != NIL_PROC) {
		rdy_tail[user_proc_group_queue]->p_nextready = rdy_head[user_proc_group_queue];
		rdy_tail[user_proc_group_queue] = rdy_head[user_proc_group_queue];
		rdy_head[user_proc_group_queue] = rdy_head[user_proc_group_queue]->p_nextready;
		rdy_tail[user_proc_group_queue]->p_nextready = NIL_PROC;
  }
  if (current_group == USER_PROC_GROUP_IO){
  	current_group = USER_PROC_GROUP_CALC;
  	counter_io = 1;
  }
  if (current_group == USER_PROC_GROUP_CALC && counter_calc == 0){
  	counter_calc = 2;
  	current_group = USER_PROC_GROUP_INTER;
  }
  else if (current_group == USER_PROC_GROUP_CALC && counter_calc != 0){
  	counter_calc--;
  }
  if (current_group == USER_PROC_GROUP_INTER && counter_inter == 0){
  	counter_inter = 4;
  	current_group = USER_PROC_GROUP_IO;
  }
  else if (current_group == USER_PROC_GROUP_INTER && counter_inter != 0){
  	counter_inter--;
  }
...
/*===========================================================================*
 *				get_proc_group_queue				     *
 *===========================================================================*/
PUBLIC int get_proc_group_queue(proc_group)
register int proc_group;
{
  return proc_group + USER_Q_IO;
}

...
PUBLIC int dequeue(struct Queue* q){
...
PUBLIC void enqueue(struct Queue* q, int process){
	struct Node *var;
	var->process = process;
	if(q->front == NULL){
		q->front = var;
...
PUBLIC int queue_size(struct Queue* q){
	return q->size;
}

PUBLIC void clear_queue(struct Queue *q){
	q->rear = NULL;
	q->front = NULL;
	q->size = 0;
}
...
PUBLIC int do_sem_init(){
	int value = mm_in.m1_i1;
	semaphore->is_valid = 1;
	semaphore->value = value;
	semaphore->q = init_queue();
	return OK;
}

PUBLIC int do_sem_down(){
	int source = mm_in.m_source;
...
PUBLIC int do_sem_up(){
	message m;
	semaphore->value++;
	if (queue_size(semaphore->q) > 0){
		semaphore->value--;
		m.m_type = OK;
		m.m_source = dequeue(semaphore->q);
		if (m.m_source != -1){
			send(m.m_source, &m);
		}
	}
...
PUBLIC int do_sem_release(){
	semaphore->is_valid = 0;
	semaphore->value = 0;
	semaphore->q = NULL;
	return OK;
}

PUBLIC int do_sem_check(){
	return semaphore->value;
}

PUBLIC int do_sem_queue_check(){
	return queue_size(semaphore->q);
}
